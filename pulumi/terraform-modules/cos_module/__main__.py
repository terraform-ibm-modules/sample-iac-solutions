...
if os.path.isdir(STATIC_DIR):
    ...
    for fname in files:
        ibm.cos_bucket_object.CosBucketObject(
            f"file-{fname}",
            bucket_crn=cos.bucket_crn.apply(
                lambda crns: crns[0] if isinstance(crns, list) else crns
            ),
...
ibm.IamAccessGroupPolicy(
    ...
    resources={
        ...
        "resource_instance_id": cos.cos_instance_guid.apply(
...
pulumi.export(
    ...
    cos.bucket_crn.apply(
        lambda crn: f"https://{BUCKET_NAME}.s3.{REGION}.cloud-object-storage.appdomain.cloud"
    ),
)
